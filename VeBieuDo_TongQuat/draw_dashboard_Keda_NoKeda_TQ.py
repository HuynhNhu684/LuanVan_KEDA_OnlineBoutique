...
df_locust_no = load_and_clean_locust('TongQuat_NoKeda_stats_history.csv')
df_locust_with = load_and_clean_locust('TongQuat_WithKeda_stats_history.csv')

fe_no, pc_no = load_hpa_log('Log_NoKeda_TQ.txt')
...
def add_phase_bg(ax, show_labels=False):
    ...
    if show_labels:
        ...
        ax.text((P4_START + P4_END)/2, y_pos, 'uan sát thu hồi\n(1 User)', transform=ax.get_xaxis_transform(), fontsize=10, color='#6a1b9a', fontweight='bold', ha='center', va='center', bbox=bbox_props)

# ==========================================
# 3. VẼ SIÊU DASHBOARD 4 Ô
# ==========================================
fig = plt.figure(figsize=(24, 18)) 
fig.suptitle('BIỀU ĐỒ SO SÁNH HIỆU NĂNG VÀ KHẢ NĂNG PHẢN ỨNG KEDA - NOKEDA', fontsize=22, fontweight='bold', y=0.96)
...
add_phase_bg(ax_rps)
if not df_locust_no.empty: ax_rps.plot(df_locust_no['Time_sec'], df_locust_no['Requests/s'], color=COLOR_NO, linewidth=2.5, linestyle='--', label='Kịch bản NoKEDA (Nghẽn tải)')
...
add_phase_bg(ax_lat)
if not df_locust_no.empty: ax_lat.plot(df_locust_no['Time_sec'], df_locust_no[col_latency], color=COLOR_NO, linewidth=2.5, label='Kịch bản NoKEDA (Vọt đỉnh)')
...
def draw_server_panel(gs_pos, df_no, df_with, title):
    ...
    # 1. Vẽ RPS
    if not df_no.empty:
        ax_c.plot(df_no['Time_sec'], df_no['RPS'], color=COLOR_NO, linewidth=2.5, linestyle='--', label='RPS (NoKEDA)')
    if not df_with.empty:
        ax_c.plot(df_with['Time_sec'], df_with['RPS'], color=COLOR_WITH, linewidth=2.5, label='RPS (KEDA)')
    ...
    ax_err = ax_c.twinx()
    if not df_no.empty:
        ax_err.fill_between(df_no['Time_sec'], df_no['Error'], color=COLOR_NO, alpha=0.3, label='Lỗi (NoKEDA)')
        ax_err.plot(df_no['Time_sec'], df_no['Error'], color=COLOR_NO, linewidth=1.5)
    if not df_with.empty:
        ax_err.fill_between(df_with['Time_sec'], df_with['Error'], color=COLOR_WITH, alpha=0.4, label='Lỗi (KEDA)')
    ...
    if not df_no.empty:
        ax_p.plot(df_no['Time_sec'], df_no['Pods'], color=COLOR_POD_NO, linewidth=2.5, linestyle='--', alpha=0.9, drawstyle='steps-post', label='Số Pod (NoKEDA)')
...
draw_server_panel((1, 0), fe_no, fe_with, 'SO SÁNH REQUEST/S VÀ TỶ LỆ LỖI CỦA KEDA - NOKEDA TRÊN FRONTEND')
draw_server_panel((1, 1), pc_no, pc_with, 'SO SÁNH REQUEST/S VÀ TỶ LỆ LỖI CỦA KEDA - NOKEDA TRÊN PRODUCT CATALOG')

# 🔥 XUẤT CẢ 2 FILE CHO NHU 🔥
plt.savefig('Master_Dashboard_NoKEDA_vs_KEDA_TQ.png', dpi=300, bbox_inches='tight')
plt.savefig('Master_Dashboard_NoKEDA_vs_KEDA_TQ.svg', format='svg', bbox_inches='tight')
plt.close()
