...
def load_hpa_log(filename):
    ...
    Sử dụng Regex để bóc tách RPS (xử lý cả định dạng có 'm' milli-requests).
    """
    data = []
    try:
        try:
        ...
        for line in lines:
            ...
            # 3. Lấy thông số RPS bằng Regex (Tìm mẫu 11130m/37 hoặc 73/37 hoặc <unknown>/37)
            rps_match = re.search(r'(<unknown>|\d+m?)/\d+', line)
            if rps_match:
                val_str = rps_match.group(1)
                if 'unknown' in val_str.lower():
                    continue # Bỏ qua khoảnh khắc K8s chưa kịp đo
                
                # Xử lý Milli-requests (vd: 11130m -> 11.13 RPS)
                if val_str.endswith('m'):
                    rps_val = float(val_str.replace('m', '')) / 1000.0
                else:
                    rps_val = float(val_str)
            else:
                continue
            ...
            data.append({'Datetime': date_str, 'Service': service, 'RPS': rps_val, 'Pods': pod_val})
    ...
    # Tách dữ liệu của 2 Microservices
...
# ==========================================
print("Đang nạp dữ liệu Tối thượng (Locust + Log API RPS)...")

# Dữ liệu Locust (Client) - Cập nhật tên file RPS
df_locust_no = load_and_clean_locust('No_KEDA_RPS_stats_history.csv')
df_locust_with = load_and_clean_locust('With_KEDA_RPS_stats_history.csv')

# Dữ liệu Hạ tầng trực tiếp từ Log (Server) - Cập nhật tên file
fe_no, pc_no = load_hpa_log('Log_Tinh_Standard_RPS.txt')
fe_with, pc_with = load_hpa_log('Log_KEDA_Dong.txt')

col_latency = 'Inst_Latency'
col_users = 'User Count'

# Đồng bộ mốc thời gian lớn nhất
...
fig.suptitle('BIỂU ĐỒ SO SÁNH HIỆU NĂNG KHI CÓ KEDA VÀ CHỨNG MINH KHẢ NĂNG SCALE UP - SCALE DOWN LINH HOẠT (KỊCH BẢN RPS)', fontsize=22, fontweight='bold', y=0.95)
...
global_max_rps = max([df['RPS'].max() if not df.empty else 0 for df in all_dfs])
GLOBAL_Y_LIMIT_RPS = global_max_rps * 1.3 # Tự động tạo khoảng trống 30% phía trên cho Legend

global_max_pods = max([df['Pods'].max() if not df.empty else 1 for df in all_dfs])
GLOBAL_Y_LIMIT_PODS = int(global_max_pods) + 2

# --- HÀM VẼ GRAPH CHO POD (HỖ TRỢ TRUYỀN NGƯỠNG THRESHOLD) ---
def draw_server_panel(gs_pos, df_no, df_with, title, threshold):
    inner_gf = outer_gs[gs_pos].subgridspec(2, 1, height_ratios=[1.8, 1], hspace=0.1)
    
    # 3.1 Biểu đồ RPS Target
    ax_c = fig.add_subplot(inner_gf[0])
    add_phase_bg(ax_c, show_labels=False) 
    
    if not df_no.empty:
        ax_c.plot(df_no['Time_sec'], df_no['RPS'], color=COLOR_NO, linewidth=2, linestyle='--', alpha=0.8, label='RPS (Kịch bản NoKEDA)')
    if not df_with.empty:
        ax_c.plot(df_with['Time_sec'], df_with['RPS'], color=COLOR_WITH, linewidth=2.5, label='RPS (Kịch bản WithKEDA)')
        ax_c.fill_between(df_with['Time_sec'], df_with['RPS'], color=COLOR_WITH, alpha=0.08)
        
    ax_c.axhline(threshold, color=COLOR_THRESH, linestyle=':', linewidth=2, label=f'Ngưỡng Scale ({threshold} RPS)')
    ax_c.set_title(f'{title}', fontsize=16, fontweight='bold', pad=10)
    ax_c.set_ylabel('Target RPS / Pod', fontsize=12, fontweight='bold')
    
    ax_c.set_ylim(0, GLOBAL_Y_LIMIT_RPS)
...
# --- GỌI HÀM VẼ (TRUYỀN ĐÚNG NGƯỠNG SCALE 37 VÀ 83) ---
draw_server_panel((1, 0), fe_no, fe_with, 'SỐ LƯỢNG REQUEST/S CỦA FRONTEND NHẬN ĐƯỢC', 37)
draw_server_panel((1, 1), pc_no, pc_with, 'SỐ LƯỢNG REQUEST/S CỦA PRODUCT CATALOG NHẬN ĐƯỢC', 83)

# XUẤT RA ĐỊNH DẠNG VECTOR
plt.savefig('Master_Dashboard_RPS_Final.png', dpi=300, bbox_inches='tight')
plt.savefig('Master_Dashboard_RPS_Final.svg', format='svg', bbox_inches='tight')
plt.close()

print("-> VẼ BIỂU ĐỒ HOÀN TẤT! Đã lưu ảnh Master_Dashboard_RPS_Final.png")