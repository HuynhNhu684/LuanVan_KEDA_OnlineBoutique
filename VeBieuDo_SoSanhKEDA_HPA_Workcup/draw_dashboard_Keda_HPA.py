...
# ==========================================
print("🚀 Đang đồng bộ hóa và chuẩn bị xuất file chất lượng cao (SVG)...")

df_locust_no = load_and_clean_locust('Workcup_OnlyHpa_stats_history.csv')
df_locust_with = load_and_clean_locust('Workcup_WithKeda_stats_history.csv')

fe_no, pc_no = load_hpa_log('Log_HPA.txt')
...
# 🔥 TÌM MAX RPS CHUNG CHO 2 Ô SERVER ĐỂ ĐỒNG BỘ TRỤC Y 🔥
...
def add_phase_bg(ax, show_labels=False):
    ...
    if show_labels:
        ...
        y_pos = 0.5 
        # 🔥 CẬP NHẬT NHÃN KỊCH BẢN WORLD CUP CHO NHU 🔥
...
fig.suptitle('BIỂU ĐỒ SO SÁNH HIỆU NĂNG VÀ KHẢ NĂNG PHẢN ỨNG KEDA - HPA', fontsize=22, fontweight='bold', y=0.96)
...
add_phase_bg(ax_rps)
if not df_locust_no.empty: ax_rps.plot(df_locust_no['Time_sec'], df_locust_no['Requests/s'], color=COLOR_NO, linewidth=2.5, label='Kịch bản HPA (Tĩnh)')
if not df_locust_with.empty: ax_rps.plot(df_locust_with['Time_sec'], df_locust_with['Requests/s'], color=COLOR_WITH, linewidth=2.5, label='Kịch bản KEDA (Động)')
...
def draw_server_panel(gs_pos, df_no, df_with, rps_target, title):
    ...
    if not df_no.empty:
        ax_c.plot(df_no['Time_sec'], df_no['CPU'], color=COLOR_NO, linewidth=2.5, label='Tham số HPA (CPU %)')
        ax_c.fill_between(df_no['Time_sec'], df_no['CPU'], color=COLOR_NO, alpha=0.1) 
    ax_c.axhline(CPU_TARGET, color='darkred', linestyle=':', linewidth=1.5, label=f'Ngưỡng HPA Scale ({int(CPU_TARGET)}%)')
    ax_c.set_title(f'[SERVER] {title}', fontsize=16, fontweight='bold', pad=10)
    ax_c.set_ylabel('CPU Usage (%) - HPA', fontsize=12, fontweight='bold', color=COLOR_NO)
    ax_c.set_ylim(0, max([df['CPU'].max() if not df.empty else 100 for df in [df_no, df_with]]) * 1.25)
    
    ax_rps_in = ax_c.twinx()
    if not df_with.empty:
        ax_rps_in.plot(df_with['Time_sec'], df_with['RPS'], color='darkorchid', linewidth=2.5, label='Tham số KEDA (RPS)')
        ax_rps_in.fill_between(df_with['Time_sec'], df_with['RPS'], color='darkorchid', alpha=0.1)
    ax_rps_in.axhline(rps_target, color='purple', linestyle='--', linewidth=1.5, label=f'Ngưỡng KEDA Scale ({int(rps_target)} RPS)')
    ax_rps_in.set_ylabel('RPS Nội Bộ - KEDA', fontsize=12, fontweight='bold', color='darkorchid')
    
    # 🔥 ÉP TRỤC Y DÙNG CHUNG GLOBAL MAX ĐỂ THẤY FRONTEND THẤP HƠN PRODUCT 🔥
    ax_rps_in.set_ylim(0, GLOBAL_MAX_RPS * 1.25)
    ...
    ax_p.legend(loc='upper right', fontsize=11, framealpha=0.8)
    ax_p.grid(True, linestyle=':', alpha=0.6); ax_p.set_xlim(0, P4_END)

draw_server_panel((1, 0), fe_no, fe_with, FE_KEDA_RPS_TARGET, 'SO SÁNH PHẢN ỨNG: MICROSERVICE FRONTEND')
draw_server_panel((1, 1), pc_no, pc_with, PC_KEDA_RPS_TARGET, 'SO SÁNH PHẢN ỨNG: MICROSERVICE PRODUCT CATALOG')

# 🔥 XUẤT CẢ 2 FILE CHO NHU 🔥
plt.savefig('Master_Dashboard_HPA_KEDA_WorldCup.png', dpi=300, bbox_inches='tight')
plt.savefig('Master_Dashboard_HPA_KEDA_WorldCup.svg', format='svg', bbox_inches='tight')
plt.close()

print("✅ XONG RỒI NHU ƠI! Đã đồng bộ trục RPS và cập nhật nhãn User World Cup hoàn hảo cho em rồi nha!")